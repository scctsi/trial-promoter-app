...
RSpec.describe DataReportMapper do
  ...
  it 'maps the message total clicks for day 1 to total_clicks_day_1' do
    expect(DataReportMapper.total_clicks_day_1(@message)).to eq(3)
  end

  it 'maps the message total clicks for day 2 to total_clicks_day_2' do
    expect(DataReportMapper.total_clicks_day_2(@message)).to eq(1)
  end

  it 'maps the message total clicks for day 3 to total_clicks_day_3' do
    expect(DataReportMapper.total_clicks_day_3(@message)).to eq(2)
  end

  it 'maps the message total human clicks for the entire experiment to total_clicks_experiment' do
    expect(DataReportMapper.total_clicks_experiment(@message)).to eq(6)
  end

  it 'maps the times of each human click per tracking link to click_time' do
    expect(DataReportMapper.click_time(@message)).to eq([["12:23:13", "12:23:13", "12:23:13"], ["13:44:56"], ["19:26:01", "19:26:01"]])
  end
  ...
  describe 'impressions by day' do
    ...
    describe 'for backdated Twitter ads' do
      ...
      it 'maps the total impressions for day 1 to total_impressions_day_1 (using impressions data 5 days from the scheduled_date_time)' do
        expect(DataReportMapper.total_impressions_day_1(@message)).to eq(100)
      end
      ...
      it 'maps the total impressions to day 2 to total_impressions_day_2 (using impressions data 6 days from the scheduled_date_time)' do
        expect(DataReportMapper.total_impressions_day_2(@message)).to eq(115)
      end
      ...
      it 'maps the total impressions to day 3 to total_impressions_day_3 (using impressions data 7 days from the scheduled_date_time)' do
        expect(DataReportMapper.total_impressions_day_3(@message)).to eq(120)
      end
    end
    ...
    it 'returns NDA for data that is missing' do
      ...
      expect(DataReportMapper.total_impressions_day_1(@message)).to eq('NDA')
      @message.medium = :ad
      expect(DataReportMapper.total_impressions_day_1(@message)).to eq('NDA')
    end

    it 'maps the total impressions to day 2 to total_impressions_day_2' do
      expect(DataReportMapper.total_impressions_day_2(@message)).to eq(15)
      @message.medium = :ad
      expect(DataReportMapper.total_impressions_day_2(@message)).to eq(115)
    end
    ...
    it 'returns NDA for data that is missing on day 1 and displays impressions if data exists for day 2' do
      ...
      expect(DataReportMapper.total_impressions_day_1(@message)).to eq('NDA')
      expect(DataReportMapper.total_impressions_day_2(@message)).to eq(0)
    ...
    it 'returns NDA for data that is missing for either day 1 or day 2' do
      @message.impressions_by_day = {}
      expect(DataReportMapper.total_impressions_day_2(@message)).to eq('NDA')
      @message.medium = :ad
      expect(DataReportMapper.total_impressions_day_2(@message)).to eq('NDA')

      @message.impressions_by_day = {(@message.scheduled_date_time + 1.day).to_date => 1 }

      expect(DataReportMapper.total_impressions_day_2(@message)).to eq(1)
      @message.medium = :ad
      expect(DataReportMapper.total_impressions_day_2(@message)).to eq(1)
    end
    ...
    it 'maps the total impressions for each day to total_impressions_day_3' do
      ...
      expect(DataReportMapper.total_impressions_day_3(@message)).to eq(5)
      @message.medium = :ad
      expect(DataReportMapper.total_impressions_day_3(@message)).to eq(120)
    end
    ...
    it 'displays impressions for day 3 even if data is missing for days 1 and/or 2' do
      ...
      expect(DataReportMapper.total_impressions_day_3(@message)).to eq(0)
      ...
      expect(DataReportMapper.total_impressions_day_3(@message)).to eq('NDA')
    end
    ...
    it 'returns 0 for organic, NDA for ad if data is missing for day 3' do
      ...
      expect(DataReportMapper.total_impressions_day_3(@message)).to eq(1)
      @message.medium = :ad
      expect(DataReportMapper.total_impressions_day_3(@message)).to eq(1)
    end
  end
