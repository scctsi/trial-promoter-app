...
RSpec.describe DataReportMapper do
  ...
  it 'maps the social_network_id to social_network_id' do
    @message.social_network_id = 'social_network_id'
  ...
  describe 'experiment variables mapping methods' do
    it 'returns the value of an experiment variable when there is no value mapping (a mapping that maps the actual value of an experiment variable to a numerical value, usually used for statistical analysis)' do
      @message.message_template.experiment_variables['stem_id'] = 'FE51'
      
      expect(@data_report_mapper.experiment_variable(@message, 'stem_id')).to eq('FE51')
    end

    it 'returns the mapped value of an experiment variable when a value mapping is provided (a mapping that maps the actual value of an experiment variable to a numerical value, usually used for statistical analysis)' do
      theme_mapping = { 'health' => '1', 'appearance' => '2', 'money' => '3', 'love of family' => '4', 'addiction' => '5', 'health + community' => '6', 'health + family' => '7', 'UNCLEAR' => 'UNCLEAR' }

      theme_mapping.each do |theme, value|
        @message.message_template.experiment_variables['theme'] = theme
        expect(@data_report_mapper.experiment_variable(@message, 'theme', theme_mapping)).to eq(value)
      end
    end
  end

  it 'maps the message template content (without the {url} parameter) to variant' do
  ...
  it 'maps the social media platform to sm_type' do
  ...
  it 'maps the scheduled date of the message to day_experiment (ordinal day of the experiment: 1 for first day of experiment, 2 for second day of experiment etc)' do
  ...
  it 'uses the original_scheduled_date_time for calculating the day_experiment for backdated messages' do
    @message.medium = :ad
    @message.platform = :twitter
    @message.backdated = true
  ...
  it 'maps the date that the message was scheduled to be sent on to date_sent' do
  ...
  it 'returns the original date that the message was scheduled to be sent on for backdated messages' do
    @message.medium = :ad
    @message.platform = :twitter
    @message.backdated = true
    @message.original_scheduled_date_time = ActiveSupport::TimeZone.new("America/Los_Angeles").local(2017, 5, 5, 12, 0, 0)
  ...
  it 'maps the day of the week that the message was scheduled to be send on to day_sent' do
  ...
  it 'uses the original_scheduled_date_time when mapping the day of the week the message was sent to day_sent' do
    @message.medium = :ad
    @message.platform = :twitter
    @message.backdated = true
  ...
  it 'maps the time the message was sent to time_sent' do
  ...
  it 'maps the message medium to medium' do
  ...
  it 'maps whether an image was present to image_included' do
  ...
  describe 'clicks and click times' do
    ...
    it 'maps the message total clicks for day 1 to total_clicks_day' do
      expect(@data_report_mapper.clicks_for_day(@message).count).to eq(6)
      expect(@data_report_mapper.clicks_for_day(@message, 0).count).to eq(6)
    end
  
    it 'maps the message total clicks for day 2 to total_clicks_day' do
      expect(@data_report_mapper.clicks_for_day(@message, 1).count).to eq(6)
    end
  
    it 'maps the message total clicks for day 3 to total_clicks_day' do
      expect(@data_report_mapper.clicks_for_day(@message, 2).count).to eq(3)
    end
  
    it 'maps the message total human clicks for the entire experiment to total_clicks_experiment' do
      expect(@data_report_mapper.clicks_for_experiment(@message).count).to eq(15)
    end
  
    it 'maps the times of each kind of click (human, spider) per tracking link to click_time' do
      clicks_day_1 = @data_report_mapper.clicks_for_day(@message, 0)
      clicks_day_2 = @data_report_mapper.clicks_for_day(@message, 1)
      clicks_day_3 = @data_report_mapper.clicks_for_day(@message, 2)
      
      clicks_entire_experiment = @data_report_mapper.clicks_for_experiment(@message)
      
      expect(@data_report_mapper.click_times(clicks_entire_experiment, true, true)).to eq(["12:23:13", "12:23:13", "12:23:13", "13:44:56", "19:26:01", "19:26:01", "10:26:01", "10:26:01"])
      expect(@data_report_mapper.click_times(clicks_entire_experiment)).to eq(["12:23:13", "12:23:13", "12:23:13", "13:44:56", "19:26:01", "19:26:01", "10:26:01", "10:26:01"])
      expect(@data_report_mapper.click_times(clicks_entire_experiment, false, false)).to eq(["12:34:57", "12:34:57", "11:38:03"])
      expect(@data_report_mapper.click_times(clicks_day_1, true, true)).to eq(["12:23:13", "12:23:13", "12:23:13"])
      expect(@data_report_mapper.click_times(clicks_day_1, true, false)).to eq(["01:23:13"])
      expect(@data_report_mapper.click_times(clicks_day_1, false, true)).to eq(["11:34:57"])
      expect(@data_report_mapper.click_times(clicks_day_1, false, false)).to eq(["12:34:57"])
      expect(@data_report_mapper.click_times(clicks_day_2, true, true)).to eq(["13:44:56", "19:26:01", "19:26:01"])
      expect(@data_report_mapper.click_times(clicks_day_2, true, false)).to eq(["14:44:56"])
      expect(@data_report_mapper.click_times(clicks_day_2, false, true)).to eq(["12:34:57"])
      expect(@data_report_mapper.click_times(clicks_day_2, false, false)).to eq(["12:34:57"])
      expect(@data_report_mapper.click_times(clicks_day_3, true, true)).to eq(["10:26:01", "10:26:01"])
      expect(@data_report_mapper.click_times(clicks_day_3, true, false)).to eq([])
      expect(@data_report_mapper.click_times(clicks_day_3, false, true)).to eq([])
      expect(@data_report_mapper.click_times(clicks_day_3, false, false)).to eq(["11:38:03"])
    end
  end
  ...
  describe 'impressions by day' do
    ...
    describe 'for backdated Twitter ads' do
      ...
      it 'maps the total impressions for day 1 to impressions_for_day (using the original_scheduled_date_time as the first day)' do
        expect(@data_report_mapper.impressions_for_day(@message, 0)).to eq(100)
      end
  
      it 'maps the total impressions to day 2 to impressions_for_day (using the original_scheduled_date_time as the first day)' do
        expect(@data_report_mapper.impressions_for_day(@message, 1)).to eq(115)
      end

      it 'maps the total impressions to day 3 to impressions_for_day (using the original_scheduled_date_time as the first day)' do
        expect(@data_report_mapper.impressions_for_day(@message, 2)).to eq(120)
      end
    end

    it 'maps the total impressions for day 1 to impressions_for_day' do
      expect(@data_report_mapper.impressions_for_day(@message, 0)).to eq(100)
    ...
    it 'returns NDA for data that is missing' do
      ...
      expect(@data_report_mapper.impressions_for_day(@message, 0)).to eq('NDA')

      @message.medium = :ad
      expect(@data_report_mapper.impressions_for_day(@message, 0)).to eq('NDA')
    end

    it 'maps the total impressions to day 2 to impressions_for_day' do
      expect(@data_report_mapper.impressions_for_day(@message, 1)).to eq(15)
    ...
    it 'returns NDA for data that is missing on day 1 and displays impressions if data exists for day 2' do
      ...
      expect(@data_report_mapper.impressions_for_day(@message, 0)).to eq('NDA')
      expect(@data_report_mapper.impressions_for_day(@message, 1)).to eq(0)
    ...
    it 'returns NDA for data that is missing for either day 1 or day 2' do
      @message.impressions_by_day = {}
      expect(@data_report_mapper.impressions_for_day(@message, 1)).to eq('NDA')

      @message.medium = :ad
      expect(@data_report_mapper.impressions_for_day(@message, 1)).to eq('NDA')

      @message.impressions_by_day = {(@message.scheduled_date_time + 1.day).to_date => 1 }
      expect(@data_report_mapper.impressions_for_day(@message, 1)).to eq(1)

      @message.medium = :ad
      expect(@data_report_mapper.impressions_for_day(@message, 1)).to eq(1)
    end

    it 'maps the total impressions for day 3 to impressions_for_day' do
      @message.medium = :organic
      expect(@data_report_mapper.impressions_for_day(@message, 2)).to eq(5)
    ...
    it 'displays impressions for day 3 even if data is missing for days 1 and/or 2' do
      ...
      expect(@data_report_mapper.impressions_for_day(@message, 2)).to eq(0)
      ...
      expect(@data_report_mapper.impressions_for_day(@message, 2)).to eq('NDA')
    end
    ...
    it 'maps the total impressions for the duration of the experiment for each platform to total_impressions_experiment or return NDA' do
      expect(@data_report_mapper.impressions_for_experiment(@message_twitter)).to eq(1394)
      expect(@data_report_mapper.impressions_for_experiment(@message_facebook)).to eq(1259) 
      expect(@data_report_mapper.impressions_for_experiment(@message_instagram)).to eq(259)
      
      @message.metrics = []
      
      expect(@data_report_mapper.impressions_for_experiment(@message)).to eq("NDA") 
    end
    ...
    it 'returns 0 for organic, NDA for ad if data is missing for day 3' do
      ...
      expect(@data_report_mapper.impressions_for_day(@message, 2)).to eq(1)
      @message.medium = :ad
      expect(@data_report_mapper.impressions_for_day(@message, 2)).to eq(1)
    end
  end
  ...
  describe "sessions and conversions" do
    before do
    ...
    it 'maps the number of sessions on day 1 to sessions_for_day' do
      expect(@data_report_mapper.sessions_for_day(@message, 0, @experiment.ip_exclusion_list)).to eq(3)
    end
  
    it 'maps the number of sessions on day 2 to sessions_for_day' do
      expect(@data_report_mapper.sessions_for_day(@message, 1, @experiment.ip_exclusion_list)).to eq(2)
    end
  
    it 'maps the number of sessions on day 3 to sessions_for_day' do
      create(:visit, utm_content: @message.to_param, started_at: @message.scheduled_date_time + 2.day + 1.hour)
      expect(@data_report_mapper.sessions_for_day(@message, 2, @experiment.ip_exclusion_list)).to eq(1)
    end
  
    it 'ignores sessions that originated from a list of IP addresses' do
      create(:visit, utm_content: @message.to_param, started_at: @message.scheduled_date_time + 2.day + 1.hour, ip: '128.125.77.139')

      expect(@data_report_mapper.sessions_for_day(@message, 2, @experiment.ip_exclusion_list)).to eq(0)
    end
  
    it 'maps the number of sessions for the whole experiment to sessions_for_experiment' do
      expect(@data_report_mapper.sessions_for_experiment(@message, @experiment.ip_exclusion_list)).to eq(5)
    end
  ...
  it 'maps the time on each webpage for each website to time_on_page' do
