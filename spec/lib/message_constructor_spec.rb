...
RSpec.describe MessageConstructor do
  ...
  it 'does not try to replace the {url} variable if {url} is missing in the content of the message_template' do
    ...
    message = @message_constructor.construct(@experiment, message_template, :facebook, :ad)
  ...
  it 'sets a date and time for the message' do
    ...
    message = @message_constructor.construct(@experiment, message_template, :facebook, :ad, DateTime.new(2017, 1, 1), DateTime.new(2017, 1, 1, 1, 30, 0))
