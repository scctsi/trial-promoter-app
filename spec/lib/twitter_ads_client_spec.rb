...
RSpec.describe TwitterAdsClient do
  ...
  describe "run ad account tests on the actual ad account" do
    ...
    it 'creates a scheduled tweet' do
      message = build(:message, social_network_id: '822248659043520512', scheduled_date_time: DateTime.new(2018,3,24,4,5,6).iso8601)
      VCR.use_cassette 'twitter_ads_client/create_scheduled_tweet' do
        scheduled_tweet = @twitter_ads_client.create_scheduled_tweet(@twitter_ads_client.account, message)
        
        expect(scheduled_tweet.id).to eq(977309380042407936)
      end
    end
    
    it 'associates a line_item with a scheduled tweet' do
      scheduled_tweet_id = 822248659043520512
      line_item_id = ''
      
      VCR.use_cassette 'twitter_ads_client/create_ad_line_item_for_scheduled_tweet' do
        line_item_params = {
          name: 'my first objective',
          product_type: 'PROMOTED_TWEETS',
          placements: 'ALL_ON_TWITTER',
      ...
      VCR.use_cassette 'twitter_ads_client/create_scheduled_promoted_tweet' do
        scheduled_promoted_tweet = @twitter_ads_client.create_scheduled_promoted_tweet(@twitter_ads_client.account, line_item_id, scheduled_tweet_id)
        
        expect(scheduled_promoted_tweet.id).to eq("1tnlwh")
      end
    end
