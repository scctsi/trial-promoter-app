...
RSpec.describe MetricsManager do
  before do
    ...
    @posts = create_list(:post, 4, experiment: experiment, post_template: post_template)
  end

  context 'for messages' do
  ...
  context 'for posts' do
    it 'gets the value for a metric given a post, source of the metric and the metric name' do
      data = { @posts[0].to_param => { 'metric_1' => 1, 'metric_2' => 2}, @posts[1].to_param => { 'metric_1' => 2, 'metric_2' => 3}, @posts[2].to_param => { 'metric_1' => 3, 'metric_2' => 4} }
      MetricsManager.update_metrics(data, :google_analytics, Post)
  
      metric_value = MetricsManager.get_metric_value(@posts[0], :google_analytics, 'metric_1')
    ...
    it 'returns N/A if it cannot find a metric with the given source' do
      ...
      metric_value = MetricsManager.get_metric_value(@posts[0], :facebook, 'metric_1')
    ...
    it 'returns N/A if it cannot find a metric with the given metric name' do
      ...
      metric_value = MetricsManager.get_metric_value(@posts[0], :google_analytics, 'unknown_metric')
  
      expect(metric_value).to eq('N/A')
    end
  
    it 'updates metrics for posts given a hash of metric hashes indexed by post to_param values' do
      data = { @posts[0].to_param => { 'metric_1' => 1, 'metric_2' => 2}, @posts[1].to_param => { 'metric_1' => 2, 'metric_2' => 3}, @posts[2].to_param => { 'metric_1' => 3, 'metric_2' => 4} }
  
      MetricsManager.update_metrics(data, :google_analytics, Post)
  
      @posts.each { |post| post.reload }
      @posts[0..2].each { |post| expect(post.metrics.count).to eq(1) }
      @posts[0..2].each { |post| expect(post.metrics[0].source).to eq(:google_analytics) }
      expect(@posts[0].metrics[0].data).to eq({ 'metric_1' => 1, 'metric_2' => 2 })
      expect(@posts[1].metrics[0].data).to eq({ 'metric_1' => 2, 'metric_2' => 3 })
      expect(@posts[2].metrics[0].data).to eq({ 'metric_1' => 3, 'metric_2' => 4 })
    end
  end
end