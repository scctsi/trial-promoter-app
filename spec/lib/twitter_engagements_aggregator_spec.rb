...
RSpec.describe TwitterEngagementsAggregator do
  before do
    secrets = YAML.load_file("#{Rails.root}/spec/secrets/secrets.yml")
    allow(Setting).to receive(:[]).with(:twitter).and_return(secrets['twitter'])
    @twitter_engagements_aggregator = TwitterEngagementsAggregator.new
    ...
    VCR.use_cassette 'twitter_engagements_aggregator/test_setup' do
      @handle = @twitter_engagements_aggregator.get_account('BeFreeOfTobacco')
    end
  end
  ...
  describe "(development only tests)", :development_only_tests => true do
    ...
    it 'gets all the tweets' do
      VCR.use_cassette 'twitter_engagements_aggregator/get_all_tweets_for_a_handle' do
        tweets = @twitter_engagements_aggregator.get_all_tweets_for_a_handle('BeFreeOfTobacco')
    ...
    it 'gets the parent tweet for a comment' do
      VCR.use_cassette 'twitter_engagements_aggregator/status' do
        parent_tweet_id = '931668280883871744'
        
        tweet = @twitter_engagements_aggregator.get_parent_tweet_object(parent_tweet_id)
    ...
    it 'gets the comments for all tweets' do
      VCR.use_cassette 'twitter_engagements_aggregator/get_comments' do
        comments = @twitter_engagements_aggregator.get_comments
    ...
    it 'gets the direct messages for a handle' do
      direct_messages = nil
      VCR.use_cassette 'twitter_engagements_aggregator/get_all_direct_messages_for_a_handle' do
        direct_messages = @twitter_engagements_aggregator.get_all_direct_messages_for_a_handle
      end
      
      expect(direct_messages.count).to eq(19)
      expect(direct_messages.first.id).to eq(963865064729006084)
    end
    
    it 'gets the direct message object for each direct message id' do
      direct_messages = nil
      VCR.use_cassette 'twitter_engagements_aggregator/get_all_direct_messages_for_a_handle' do
        direct_messages = @twitter_engagements_aggregator.get_all_direct_messages_for_a_handle
      end
      ...
      expect(direct_messages.count).to eq(19)
      expect(direct_messages.first.id).to eq(963865064729006084)
      expect(direct_messages.first.text).to eq("Thank you for the invitation.")
    end
  end
end