...
describe Message do
  ...
  describe "#events" do
    ...
    xit "logs each event with the utm_content set to the to_param values of each message" do
      #REF https://github.com/ankane/ahoy/blob/081d97500f51f20eb2b2ba237ff6f215bbce115c/README.md#querying-properties
      events = Ahoy::Event.where(name: "Converted").where_properties(utm_content: @messages[1].to_param)



      expect(@messages[2].events.count).to eq(1)
      expect(@messages[2].events[0].utm_content).to eq(@messages[2].to_param)
    end

    xit "has a scope for finding visits that belong to a message" do
