...
RSpec.describe PublishMessagesJob, type: :job do
  ...
  before do
    ...
    (0..4).each do |index|
      ...
      @messages[index].scheduled_date = Time.now + 1.day
      @messages[index].save
    end
    (5..9).each do |index|
      @messages[index].publish_status = :published_to_buffer
      @messages[index].scheduled_date = Time.now + 30.days
      @messages[index].save
    end
  end
  ...
  it 'executes perform and publishes pending messages that need to be published to social networks in the next 7 days' do
    @messages[0].scheduled_date = Time.new(2010, 1, 1, 0, 0, 0)
    @messages[1].scheduled_date = Time.new(2010, 1, 6, 0, 0, 0)
    @messages[2].scheduled_date = Time.new(2010, 1, 8, 0, 0, 0)
    @messages[3].scheduled_date = Time.new(2010, 1, 9, 0, 0, 0)
    @messages[4].scheduled_date = Time.new(2010, 1, 10, 0, 0, 0)
