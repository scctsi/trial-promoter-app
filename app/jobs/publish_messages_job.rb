class PublishMessagesJob < ActiveJob::Base
  ...
  def perform(experiment, number_of_days = 7)
    ...
    pending_messages.all.each do |pending_message|
      BufferClient.create_update(experiment, pending_message) unless (pending_message.platform == :instagram && pending_message.medium == :organic)
